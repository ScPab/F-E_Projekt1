...
`wrappers/gdc/cache.py` / `wrappers/geo/cache.py` — bewusst nicht
importiert, damit dieser Wrapper unabhängig von den anderen bleibt (siehe
ADR-0001: jede Datenquelle ein eigenes Unterpaket). Das Interface ist
bewusst schmal gehalten, damit ein späterer Backend-Wechsel (z. B. Redis,
Objektspeicher) keine Aufrufer im Wrapper oder Mediator anfassen muss.

Tiers:
    1. Recipes      – Query-Spezifikationen (hier: ENA-Suchqueries), leicht-
    ...
    3. Raw          – Rohdaten (z. B. FASTQ-Dateien), nur transient, danach
...
`wrappers/gdc/cache.py` / `wrappers/geo/cache.py` — deliberately not
imported, so this wrapper stays independent of the others (see ADR-0001:
each data source its own sub-package). The interface is deliberately kept
narrow so a later backend change (e.g. Redis, object storage) does not need
to touch any callers in the wrapper or mediator.

Tiers:
    1. Recipes      – query specifications (here: ENA search queries),
    ...
    3. Raw          – raw data (e.g. FASTQ files), transient only, deleted
...
class RawDataCache:
    """Tier 3: Rohdaten (z. B. FASTQ-Dateien). Nur transient — nach Verarbeitung löschen.

    English: Tier 3: raw data (e.g. FASTQ files). Transient only — delete after processing.
    """
